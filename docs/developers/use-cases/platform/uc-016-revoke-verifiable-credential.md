...
- The issuer member has permission to revoke credentials for their organization
- The credential was originally issued by this issuer organization ([UC-012](/docs/developers/use-cases/platform/uc-012-issue-verifiable-credential))
- The credential is currently in `VALID` or `EXPIRED` status (not already revoked)

## Main Flow

1. The issuer member navigates to the issued credentials history in the Issuer perspective
2. The issuer member locates the credential to revoke (by credential ID, holder name/DID, or credential type)
3. The issuer member clicks **Revoke** on the credential
...
5. The issuer member enters the revocation reason and confirms
6. The backend validates the revocation request:
   - Verifies the issuer member has revocation permission
   - Verifies the credential belongs to this issuer organization
...
7. The backend submits a revocation transaction to the blockchain:
   ...
   - The transaction is signed with the issuer organization's secp256k1 key
...
10. The portal confirms the revocation to the issuer member
...
### AF-2: Issuer member lacks revocation permission
...
- The portal shows an error and the issuer member can retry

### AF-4: Credential belongs to different issuer
- At step 6, the credential was issued by a different organization
- The backend rejects the request — only the original issuer can revoke their own credentials
...
- **On-chain vs off-chain**: The revocation is recorded on-chain so that any verifier can check the revocation status without trusting the issuer's backend. The backend also stores a local copy for fast queries
...
- **Revocation reasons**: Common reasons include: "Issued in error", "Holder no longer meets requirements", "Credential information changed", "Issuer organization dissolved", "Holder requested revocation"
- **Notification**: The holder is notified via the platform (dashboard notification). Future: DIDComm v2 message via the mediator
