...
From the Issuer perspective of the dashboard, an issuer member issues a verifiable credential (VC) to a user. For example, a university issues a degree credential to a graduate. The issuer selects the credential type, fills in the claims, sets the validity period (with an optional expiration date), signs the credential with the issuer's DID key, and delivers it to the holder. The credential conforms to the W3C Verifiable Credentials Data Model. After issuance, the issuer can revoke the credential if needed ([UC-016](/docs/developers/use-cases/platform/uc-016-revoke-verifiable-credential)).

## Actors

- **Issuer Member**: Person acting from the Issuer perspective with permission to issue credentials
...
- The issuer member is authenticated and on the dashboard in **Issuer perspective** ([UC-010](/docs/developers/use-cases/platform/uc-010-view-dashboard-by-perspective))
- The issuer member has permission to issue credentials for their organization
- The issuer organization has a DID anchored on the blockchain ([UC-003](/docs/developers/use-cases/wallet/uc-003-anchor-did-on-blockchain))
- The target holder has a DID registered in the platform

## Main Flow

1. The issuer member navigates to the credential issuance section in the Issuer perspective
2. The issuer member selects a credential type/schema (e.g., "University Degree", "Professional Certification")
3. The portal displays the form with fields defined by the selected schema
4. The issuer member enters the claims:
...
5. The issuer member submits the credential
...
7. The backend signs the credential using the issuer's private key (Ed25519 or the issuer organization's key)
...
- The form shows an error and the issuer cannot proceed until a valid holder is selected

### AF-2: Issuer member lacks permission
- At step 5, the backend validates the member's role within the issuer organization
...
- At step 4, the issuer enters an expiration date that is before the issuance date or in the past
- The form shows a validation error on the expiration date field
- The issuer must correct the date before submitting

## Postconditions

- A new verifiable credential exists, signed by the issuer, with status `VALID`
...
- The issuance is recorded in the issuer's history
- The issuer can later revoke the credential if needed ([UC-016](/docs/developers/use-cases/platform/uc-016-revoke-verifiable-credential))
...
| **blockchain** | Issuer DID resolution (for key retrieval), optional credential anchoring, revocation registry |

## Technical Notes

- **Credential format**: W3C Verifiable Credentials Data Model 2.0. JSON-LD with cryptographic proof
- **Signing**: The issuer's Ed25519 key signs the credential. The key is associated with the issuer organization's DID
...
- **Delivery**: The credential is stored server-side and made available to the holder via the API. Future: delivery via DIDComm through the mediator module
- **Current implementation**: The issuer management page (`/dashboard/issuers`) allows creating issuers and managing members. Credential issuance UI does not exist yet
